...
export class PedidosPage implements OnInit {
  ...
  async presentAlertEliminar(pedido) {
    const alert = await this.alertController.create({
      header: 'Confirmar eliminación de registro',
      message: '¿Está seguro de eliminar de forma permanente el pedido?',
      buttons: [
        {
          text: 'Cancelar',
          role: 'cancel',
  ...
  eliminar(pedido){
    this.http.eliminarPedido(pedido.idPedido).then(
      (inv) => {
        console.log(inv);
